...
(function(){
	...
	var template = $('#optionsTemplate').html();
	Mustache.parse(template); 
      ...
      console.log(hash)
      if(hash){
        baseUrl = "/will/" + hash.replace("#","");
      }else{
        baseUrl = "/will" ;
      }

	var result = function(up,down){
              $("#iwill").attr("href","#" + current.code);
              $("#godie").attr("href","#" + current.code);
		$(".v_u_u").css("font-size",up <= 25?up-5 + "px":"25px");
		$(".v_d_d").css("font-size",down <= 25?down-5 + "px":"25px");
		$("#iwill").text("下一个");
              console.log(up + "," + down);
		$(".v_u_u").css("height",up + "%");
		$(".v_d_d").css("height",down + "%");
		$("#godie").text("评价此情况");
	}
	...
	content: '<div id="btn_group"><button id="btn_down">嘘</button>	<div id="btn_or"></div> <button id="btn_up">赞</button></div>',
			...
			d.show(document.getElementById('godie'));
                    $("#btn_up").click(function(){
                          $.getJSON("/voteup/"+current.code,function(data){
                            console.log(data);
                        });
                    });
                    $("#btn_down").click(function(){
                          $.getJSON("/votedown/"+current.code,function(data){
                            console.log(data);
                        });
                    });
		}else{
	...
	$("#iwill").click(function(){
		...
		if(state == "choose"){
			var willCount = parseFloat(current.i_will_count);
                   ...
                   $(".v_u_u").prepend("你和");
			state = "result";
                    $.getJSON("/iwill/"+current.code,function(data){
                            console.log(data);
                      });
		}else{
			$(".options").html("");
	...
	$("#submit").click(function(){
		$("#DIV_1").addClass("dialog-show");
	});

	$("#DIV_16").click(function(){
		$("#DIV_1").removeClass("dialog-show");
	});
